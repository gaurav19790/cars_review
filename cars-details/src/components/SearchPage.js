      ...
      <div className=" gap-2 w-full h-full">
        {search.length ? search?.filter(({name,brand})=>((name?.includes(names))||(brand?.includes(brands))||((names==null)&&(brands==null)))).map(
          (
            ...
            // curr
            <Link to={`/search/carDetail=${name.toLowerCase()}`} key={index}>
              <div className="flex border-2  gap-2 m-2 ">
                <img className="w-56 h-full" src="https://imgd.aeplcdn.com/1280x720/n/cw/ec/54399/swift-exterior-right-front-three-quarter-64.jpeg?isig=0&q=80" alt="car"/>
                 <div className=" text-center rounded-md ">
                <h1>{name} , </h1>{model} , {price} ,<p>{mileage}  ,{engine}  ,{fuel}  ,{seatingCapacity}  ,</p> 
                </div>
                </div>
             </Link>
          )
